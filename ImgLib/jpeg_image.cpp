...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    */
     ...
     */
        ...
        std::vector<JSAMPLE> dst(image.GetWidth() * 3);
        for (int x = 0; x < image.GetWidth(); x++) {
            dst[x * 3 + 0] = static_cast<JSAMPLE>(line[x].r);
            dst[x * 3 + 1] = static_cast<JSAMPLE>(line[x].g);
            dst[x * 3 + 2] = static_cast<JSAMPLE>(line[x].b);
        }
        row_pointer[0] = dst.data();
